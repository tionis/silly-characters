...
function getOAuthConfig(): OAuthConfig {
  if (!env.NEXTCLOUD_BASE_URL) {
    throw new Error("NEXTCLOUD_BASE_URL is not configured");
  }
  if (!env.NEXTCLOUD_APP_ID) {
    throw new Error("NEXTCLOUD_APP_ID is not configured");
  }
  if (!env.NEXTCLOUD_APP_SECRET) {
    throw new Error("NEXTCLOUD_APP_SECRET is not configured");
  }
  if (!env.NEXTCLOUD_REDIRECT_URI) {
    throw new Error("NEXTCLOUD_REDIRECT_URI is not configured");
  }

  return {
    baseUrl: normalizeBaseUrl(env.NEXTCLOUD_BASE_URL),
    clientId: env.NEXTCLOUD_APP_ID,
    clientSecret: env.NEXTCLOUD_APP_SECRET,
    redirectUri: env.NEXTCLOUD_REDIRECT_URI
  };
}
